...
import { Propietario, usePropietarioSelector } from "../../storage/propietarios.reducer";
...
function PropietariosTable(props) {
  ...
  const propietarios: Propietario[] = usePropietarioSelector((state) => state?.propietarios);
  ...
  onRowDelete: (oldData) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        deletePropietario(oldData.id, cookie.session.token).then((response) => {
          if (response) {
            dispatch({ type: Action.REMOVE_PROPIETARIO, propietario_id: oldData.id })
            resolve();
          }
          reject();
        })
